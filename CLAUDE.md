...
- **⚠️ A investigar — feed de saída não propaga reservas de outros canais**: `/api/ical/villa-solria.ics` e `/api/ical/export` leem **só a tabela `bookings`** (site + manuais). Reservas vindas de `booking_ical`/`airbnb_ical` vivem em `blocked_dates` e **não** saem no feed. Ou seja, a reserva Booking de 16-24 Set não é anunciada ao Airbnb/VRBO por esta via. Confirmar com o Bruno se ligou os feeds das plataformas directamente entre si (Booking → Airbnb); se não, há risco real de overbooking cruzado e o export tem de passar a incluir `blocked_dates` de outras origens.
