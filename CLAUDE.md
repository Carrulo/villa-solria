...
- **Propagação entre canais (verificado 2026-09-01)**: o feed de saída (`/api/ical/villa-solria.ics`, `/api/ical/export`) exporta **só a tabela `bookings`** — reservas de `booking_ical`/`airbnb_ical` vivem em `blocked_dates` e não saem. A propagação depende de ligações **plataforma-a-plataforma**, não do site.
  - **Booking → Airbnb: FUNCIONA** — confirmado visualmente no multicalendar do Airbnb (53089334): 16-23 Set riscados = reserva Booking 16-24. O Airbnb bloqueia internamente mas **não reexporta blocos importados** no seu `.ics` (anti-loop), por isso o nosso `blocked_dates` só vê 2 dias `airbnb_ical` — isso é normal, não é sintoma de avaria.
  - **VRBO: por confirmar** — o feed do VRBO tem um bloco 8 Ago→5 Set que casa exactamente com as reservas do site (Patricia/Bruno/Kiko), o que sugere que o VRBO **reexporta** o que importa; e não tem 16-24 Set. Verificar no calendário do VRBO se essas datas estão fechadas.
  - Se algum canal falhar, a correcção estrutural é o site passar a ser hub: feed por canal (`?exclude=<source>`) incluindo `blocked_dates` de outras origens.
