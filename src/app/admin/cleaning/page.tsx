...
export default function AdminCleaningPage() {
  ...
  async function shareCleanerLink() {
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) {
      showToast('Erro a obter token', 'error');
      return;
    }
    let token = typeof data?.value === 'string' ? data.value.trim() : '';
    if (!token) {
      // Auto-provision a token if none exists yet.
      token = crypto.randomUUID().replace(/-/g, '');
      const { error: upErr } = await supabase
        .from('settings')
        .upsert({ key: 'cleaner_token', value: token }, { onConflict: 'key' });
      if (upErr) {
        showToast('Erro a criar token', 'error');
        return;
      }
    }
    const url = `${window.location.origin}/cleaning?token=${encodeURIComponent(token)}`;
    try {
      await navigator.clipboard.writeText(url);
      showToast('Link copiado', 'success');
    } catch {
      // Fallback: prompt with the URL pre-selected so the admin can copy manually.
      prompt('Copia o link:', url);
    }
  }

  async function rotateCleanerToken() {
    if (!confirm('Gerar novo token? O link antigo deixa de funcionar.')) return;
    const token = crypto.randomUUID().replace(/-/g, '');
    const { error } = await supabase
      .from('settings')
      .upsert({ key: 'cleaner_token', value: token }, { onConflict: 'key' });
    if (error) {
      showToast('Erro a rodar token', 'error');
      return;
    }
    const url = `${window.location.origin}/cleaning?token=${encodeURIComponent(token)}`;
    try {
      await navigator.clipboard.writeText(url);
      showToast('Novo link copiado', 'success');
    } catch {
      prompt('Novo link:', url);
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={shareCleanerLink}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-600 text-white hover:bg-purple-500 transition-colors text-sm font-medium"
            title="Copiar link com token para a equipa de limpeza"
          >
            <Share2 size={14} />
            Partilhar
          </button>
          <button
            onClick={rotateCleanerToken}
            className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-white/5 text-gray-300 hover:bg-white/10 transition-colors"
            title="Gerar novo token (revoga o link antigo)"
          >
            <RotateCw size={14} />
          </button>
          <button
...
function OwnerInstructions({
  ...
  const hasInstructions = initialNotes.length > 0 || (initialRooms && initialRooms.length > 0 && initialRooms.length < villaRooms);
  ...
  return (
    <>
      ...
      >
        ...
        {initialRooms && initialRooms.length > 0 && initialRooms.length < villaRooms && (
